...
async function carregarContexto(
  ...
  if (comunicacao.status !== 'aprovada') {
    return { error: buildMensagemErro('A comunicação precisa estar aprovada antes do envio via WhatsApp', 409) }
  }
...
function validarTelefoneSelecionado(telefonesPermitidos: Array<string | null | undefined>, telefoneInformado: string) {
  ...
  if (!telefonesValidos.length) return { error: 'Nenhum telefone disponível para envio via WhatsApp.' }
...
export async function POST(
  ...
  if (destinatarioTipo === 'contato') {
    ...
    if (!data.ativo) {
      return buildMensagemErro('Contato inativo não pode receber WhatsApp.', 409)
    }
    if (!data.celular) {
  ...
  const url = buildWhatsAppUrl(telefoneSelecionado, mensagem)
  if (!url) {
    return buildMensagemErro('Não foi possível montar o link do WhatsApp.')
  }

  await registrarLog(supabase, comunicacaoId, 'whatsapp_iniciado', auth.userId, {
